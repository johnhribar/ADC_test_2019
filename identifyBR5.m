function success = identifyBR5(CH_str, sample_ratio, start, cycle, rcasic)
...
elseif (start == 9 && rcasic == 1) || (start == 1 && rcasic == 2)
        for i= start:start+3
            for n = cycle:length(CH_str)-(max_string_length-1)
                if strcmp(CH_str(i,n:n+length(two_ones_two_zeros)-1),two_ones_two_zeros)
                    count(1) = count(1) + 1;
                    break;
                end
            end
        end
elseif (start == 5 || start == 9) && rcasic == 2
        for i= start:start+3
            for n = cycle:length(CH_str)-(max_string_length-1)
